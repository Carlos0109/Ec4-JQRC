...
class LoginActivity : AppCompatActivity() {
    ...
    private fun singUpWithEmailPassword() {
    ...
    private fun signInwithGoogle() {
        val googleSignOptions = GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN).requestIdToken("108955320249943244699").requestEmail().build()
        val client: GoogleSignInClient = GoogleSignIn.getClient(this,googleSignOptions)
        val intent = client.signInIntent
        googleLauncher.launch(intent)
    }

    private fun singInWithEmailPassword() {
        val email = binding.tilEmail.editText?.text.toString()
        val password = binding.tilPassword.editText?.text.toString()
        singinFirebaseWithEmail(email,password)
    }

    private fun singinFirebaseWithEmail(email: String, password: String) {
